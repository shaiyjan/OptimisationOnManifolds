function [out1] = Ladder2(p0,v,W)
%PARATRANS Summary of this function goes here
%   Detailed explanation goes here

global hf hg hH hf1 hf2 hf3 h;
h=0.01;
target=400;
geodsteps=100;
...
df1=hessian(diff(f,x(1)));
df2=hessian(diff(f,x(2)));
df3=hessian(diff(f,x(3)));

hf1=@(p) df1(p(1),p(2),p(3));
hf2=@(p) df2(p(1),p(2),p(3));
hf3=@(p) df3(p(1),p(2),p(3));
...
if nargin==0
   ...
   W={w};
   clf;
   fsurf(@(x,y)1/(x^2+y^2), [-3 3]);
   %fsurf(@(x,y)1/(x*y), [0.1 5], 'EdgeColor', 'none');
   hold on; grid on;
   xlabel('x');
   ylabel('y');
   zlabel('z');
    ...
    if PT~=0
        ...
        elseif PT==2
            ...
            if nargin==0
               ...
               mplot=plot3(0,0,0,'Color','m');
               rplot=plot3(0,0,0,'Color','r');
               yplot=plot3(0,0,0,'Color','y');
               oplot=plot3(0,0,0,'Color',[0.9 0.6 0.2]);
               legend([cplot,yplot,mplot,rplot,oplot],'xyz=1','Geodesic v','Geodesic w','Log center','Log transport');
               xlim([0.1 5]);
