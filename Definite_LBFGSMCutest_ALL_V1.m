...
addpath('/home/gc1355/Dropbox/Diss/Data/Definite_LBFGSMCutest_ALL_V1.m')
addpath('/home/gc1355/Downloads/Cutest/Gould_Ref/cutest/src/matlab')

MainFolder="/home/gc1355/Dropbox/Diss/Data/V1-"+string(datetime('today'));
mkdir(MainFolder);
for SET=["Circ"] %["Circ","4Circ","CircInt","Stiefel"]
    destination=MainFolder+"/"+SET;
    mkdir(destination);
    for t={'ACOP','NASH'} %{'ACOP','DEGE','NASH','AVGA'}
...
function [out]= c(P,V)
global   minv dgf h restart maxstep iterations;
%variables
c1=0.1;
c2=0.9;
kap=0.8;
...
while 1
    ...
    if s<=minv+c1*ind*h*dgf & -V{ind}.'*PrTpM(cutest_grad(p),hg(p))<=-c2*dgf
        minv=s;
        %fprintf('Armijo succesfull with \n');
        out=ind;
        break
    end
    fact=fact*kap;
